...
-- amac
-- bs x 3ch x frsize x boyut1 x boyut2 ??
-- zero_loc --> zero_loc = torch.zeros(videosayisibatch,2)
-- ones = torch.ones(videosayisibatch,1,opt.glimpsePatchSize[1],opt.glimpsePatchSize[2])
-- visited_map0 = torch.zeros(opt.batchSize,1,highResSize[1],highResSize[2])
-- buraya kadar ayni
-- video dedik ama videoyu tek resim gibi dusun o videoya ait gecmis loc bilgileri
  ...
  -- output: {loc, image_next, visited_map_next}
	...
	inputs = {zero_loc, lowRes, visited_map0, ones, lowRes} -- zero_loc 
	--print('visited_map0==========',visited_map0)
	outputs = model:forward(inputs)
	print('outputs[1]====loc===>',outputs[1])
	--print('outputs[3]====visited_map_next===>',outputs[3])
	--print('outputs',outputs[2][1])
	pos =  (((outputs[1] +1 ) * (128 - 0)) / (1 +1)) + 0
	--pos1 =  ((outputs[1] +1 ) / (2))*128
	print('pos_loc==>',pos)
	NewValue_1= (((outputs[2][1] +1) * (1 - 0)) / (1 +1)) + 0
	--NewValue_2 = (((outputs[2][2] +1) * (1 - 0)) / (1 +1)) + 0
	--NewValue_3 = (((outputs[2][3] +1) * (1 - 0)) / (1 +1)) + 0
	print('inputs',inputs)
	--print(NewValue)
	--image.save('hr_patch'..tostring(epoch)..'.png',NewValue_1)
	--image.save('hr_patch'..tostring(2)..'.png',NewValue_2)
	--image.save('hr_patch'..tostring(3)..'.png',NewValue_3)
