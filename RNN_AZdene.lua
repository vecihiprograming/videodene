...
opt = lapp[[
   ...
   --nThreads                 (default 8)         # of data loading threads to use
...
function test()
  ...
  for st = 1,dataTest:size(),opt.batchSize do
    ...
    sayi=sayi+1
    os.execute("mkdir " .. opt.name .. '/' .. sayi)
    for i = 1,quantity do
      ...
      if opt.save_im then        
        ...
        img:add(1):div(2)
        image.save(opt.name..'/'..sayi .. '/' .. paths.basename(impath[i]), img)
        print('bir image kaydetti')
      end
    end
  end
